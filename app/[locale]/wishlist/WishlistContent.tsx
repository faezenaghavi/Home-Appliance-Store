...
export default function WishlistContent() {
  ...
  const mockItems = [
    { id: "1", name: 'Samsung 65" Neo QLED', nameFa: "تلویزیون هوشمند سامسونگ", price: 89900000 },
  ];
    ...
    <main dir={direction} className="min-h-screen bg-[#faf8f5]">
      <div className="px-4 sm:px-6 lg:px-8 xl:px-12 max-w-7xl mx-auto pt-24 sm:pt-28 pb-16">
        ...
        <div className="mb-10">
          ...
          <p className="text-[#8a8577] text-base">{t("wishlist.emptyDesc")}</p>
            ...
            {mockItems.map((product) => (
              <div key={product.id} className="group bg-white rounded-2xl border border-[#1a1a1a]/5 overflow-hidden hover:shadow-lg transition-all duration-300">
                <div className="relative aspect-square bg-[#faf8f5] overflow-hidden flex items-center justify-center">
                  ...
                  <button className="absolute top-3 right-3 w-8 h-8 rounded-lg bg-white/80 backdrop-blur-sm flex items-center justify-center text-red-400 hover:bg-red-50 hover:text-red-500 transition-colors shadow-sm">
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
                <div className="p-4">
                  <h3 className="font-display text-base font-semibold text-[#1a1a1a] truncate">{isRTL ? product.nameFa : product.name}</h3>
                  <p className="text-[#c4a882] font-bold mt-2">{new Intl.NumberFormat(isRTL ? 'fa-IR' : 'en-US').format(product.price)} {t("common.currency")}</p>
                </div>
              </div>
            ))}
          </div>
