...
import {
  ...
  Grid3X3, Info, Newspaper, Phone, MapPin
} from "lucide-react";
...
export default function Navbar({ cartCount = 0, wishlistCount = 0 }: NavbarProps) {
  ...
  useEffect(() => {
    const sections = ["hero", "offers", "featured", "categories", "about", "blog"];
    const handleScroll = () => {
  ...
  const navLinks = [
    { key: "nav.offers", labelFa: "تخفیف‌دارها", labelEn: "Special Offers", href: "#offers", icon: Tag, sectionId: "offers" },
    { key: "nav.featured", labelFa: "محصولات ویژه", labelEn: "Featured", href: "#featured", icon: Sparkles, sectionId: "featured" },
    { key: "nav.categories", labelFa: "دسته‌بندی محصولات", labelEn: "Categories", href: "#categories", hasDropdown: true, icon: Grid3X3, sectionId: "categories" },
    { key: "nav.about", labelFa: "درباره ما", labelEn: "About Us", href: "#about", icon: Info, sectionId: "about" },
    { key: "nav.blog", labelFa: "بلاگ", labelEn: "Blog", href: "#blog", icon: Newspaper, sectionId: "blog" },
  ];

  const userMenuItems = [
    { icon: User, labelFa: "حساب کاربری", labelEn: "My Account", href: "#account", descFa: "مشاهده و ویرایش پروفایل", descEn: "View & edit profile" },
    { icon: Package, labelFa: "سفارشات من", labelEn: "My Orders", href: "#orders", descFa: "پیگیری خریدهای شما", descEn: "Track your purchases" },
    { icon: Heart, labelFa: "علاقه‌مندی‌ها", labelEn: "Wishlist", href: "#wishlist", descFa: "محصولات ذخیره‌شده", descEn: "Saved items" },
    { icon: Settings, labelFa: "تنظیمات", labelEn: "Settings", href: "#settings", descFa: "تنظیمات حساب", descEn: "Account settings" },
  ];
  ...
  return (
    <>
      {/* Main Navbar - Removed style top offset, tightened mobile padding */}
      <nav
        dir={direction}
        ...
        <div className="px-4 sm:px-6 lg:px-8 xl:px-12 max-w-[1440px] mx-auto">
          <div className="flex items-center justify-between gap-2 sm:gap-4">
            ...
            <a href="/" className="flex items-center gap-2 sm:gap-3 shrink-0 group">
              <div
                className={`w-9 h-9 sm:w-11 sm:h-11 rounded-xl flex items-center justify-center border transition-all duration-500 ${
                  isScrolled
                    ? "bg-[#1a1a1a] border-[#1a1a1a]"
                    : "bg-white/10 border-white/25 backdrop-blur-md"
                }`}
              >
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={isScrolled ? "text-[#c4a882]" : "text-white"}>
              ...
              <div className="flex flex-col">
                <span className={`font-display text-lg sm:text-xl font-bold tracking-tight transition-colors leading-none ${isScrolled ? "text-[#1a1a1a]" : "text-white"}`}>
                  NOVIRA
                </span>
                <span className={`text-[8px] sm:text-[9px] tracking-[0.2em] sm:tracking-[0.25em] uppercase transition-colors mt-0.5 ${isScrolled ? "text-[#8a8577]" : "text-white/60"}`}>
                ...
                return (
                  <div key={link.key} ref={link.hasDropdown ? categoryRef : undefined} className="relative">
                    <a
                      ...
                      className={`relative flex items-center gap-1.5 px-4 py-2.5 rounded-full text-[13px] font-medium tracking-wide transition-all duration-300 ${
                        active
                          ? isScrolled
                            ? "text-[#c4a882]"
                            : "text-white"
                    ...
                    >
                      <span
                        className={`absolute inset-0 rounded-full transition-all duration-300 ${
                          isHovered && !active ? (isScrolled ? "bg-[#1a1a1a]/5" : "bg-white/10") : "opacity-0"
                        }`}
                      />
                      <span
                        className={`absolute inset-0 rounded-full transition-all duration-300 ${
                          active ? (isScrolled ? "bg-[#c4a882]/10" : "bg-white/15") : "opacity-0"
                        }`}
                      />
                        ...
                        <div className="p-1">
                          ...
                          <div className="grid grid-cols-2 gap-1 p-1">
                            {applianceCategories.map((cat) => (
                              <a key={cat.id} href={`#${cat.id}`} onClick={() => setIsCategoryOpen(false)} className="flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-[#faf8f5] transition-all duration-200 group">
            ...
            {/* Right Section: Search + Icons - Tighter mobile gaps */}
            <div className="flex items-center gap-1.5 sm:gap-2">

              {/* Search - visible on md (tablets) and up */}
              <div className={`hidden md:flex items-center h-9 sm:h-10 rounded-full px-4 border transition-all duration-300 w-[140px] sm:w-[160px] xl:w-[200px] ${
                isScrolled
                  ? "bg-[#1a1a1a]/[0.03] border-[#1a1a1a]/10 focus-within:border-[#c4a882]/40 focus-within:bg-[#c4a882]/[0.03] focus-within:w-[180px] sm:focus-within:w-[200px] xl:focus-within:w-[260px]"
                  : "bg-white/10 border-white/20 focus-within:bg-white/15 focus-within:border-white/30 focus-within:w-[180px] sm:focus-within:w-[200px] xl:focus-within:w-[260px]"
              }`}>
                <Search className={`w-3.5 h-3.5 shrink-0 transition-colors ${isScrolled ? "text-[#1a1a1a]/30" : "text-white/50"}`} />
                <input
                  type="text"
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  placeholder={locale === "fa" ? "جستجو..." : "Search..."}
                  className={`bg-transparent border-none outline-none text-[12px] w-full transition-all ${
                    isRTL ? "pr-2.5" : "pl-2.5"
                  } ${
                    isScrolled
                      ? "text-[#1a1a1a] placeholder:text-[#1a1a1a]/35"
              ...
              {/* Divider */}
              <span className={`hidden md:inline w-px h-5 sm:h-6 transition-colors ${isScrolled ? "bg-[#1a1a1a]/10" : "bg-white/15"}`} />

              {/* Wishlist - Responsive sizing */}
              <a href="#wishlist" className={`group relative w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center border transition-all duration-300 ${
                isScrolled
                  ? "bg-transparent border-[#1a1a1a]/10 hover:border-[#c4a882]/40 hover:bg-[#c4a882]/5"
                  : "bg-white/5 border-white/15 hover:bg-white/10 hover:border-white/25"
              }`}>
                <Heart className={`w-[16px] h-[16px] sm:w-[18px] sm:h-[18px] transition-all duration-300 group-hover:scale-110 ${isScrolled ? "text-[#1a1a1a]/70 group-hover:text-[#c4a882]" : "text-white/80 group-hover:text-white"}`} />
                {wishlistCount > 0 && (
                  <span className={`absolute -top-1.5 w-[18px] h-[18px] bg-[#c4a882] text-white text-[9px] font-bold rounded-full flex items-center justify-center shadow-sm border-2 ${isScrolled ? "border-[#faf8f5]" : "border-transparent"} ${isRTL ? "-left-1.5" : "-right-1.5"}`}>{wishlistCount}</span>
                )}
              </a>

              {/* Cart - Responsive sizing */}
              <a href="#cart" className={`group relative w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center border transition-all duration-300 ${
                isScrolled
                  ? "bg-transparent border-[#1a1a1a]/10 hover:border-[#c4a882]/40 hover:bg-[#c4a882]/5"
                  : "bg-white/5 border-white/15 hover:bg-white/10 hover:border-white/25"
              }`}>
                <ShoppingBag className={`w-[16px] h-[16px] sm:w-[18px] sm:h-[18px] transition-all duration-300 group-hover:scale-110 ${isScrolled ? "text-[#1a1a1a]/70 group-hover:text-[#c4a882]" : "text-white/80 group-hover:text-white"}`} />
                {cartCount > 0 && (
                  <span className={`absolute -top-1.5 w-[18px] h-[18px] bg-[#c4a882] text-white text-[9px] font-bold rounded-full flex items-center justify-center shadow-sm border-2 ${isScrolled ? "border-[#faf8f5]" : "border-transparent"} ${isRTL ? "-left-1.5" : "-right-1.5"}`}>{cartCount}</span>
                )}
              </a>

              {/* Language & User - Desktop (lg) only */}
              <div ref={langRef} className="relative hidden lg:block">
                <button
                  ...
                  className={`group w-10 h-10 rounded-xl flex items-center justify-center border transition-all duration-300 ${
                    isScrolled
                      ? isLangOpen
                        ? "bg-[#c4a882]/10 border-[#c4a882]/40 text-[#c4a882]"
                        : "bg-transparent border-[#1a1a1a]/10 hover:border-[#c4a882]/40 hover:bg-[#c4a882]/5"
                      : isLangOpen
                ...
                >
                  <Globe className={`w-[18px] h-[18px] transition-all duration-300 group-hover:scale-110 ${isScrolled ? "text-[#1a1a1a]/70 group-hover:text-[#c4a882]" : "text-white/80 group-hover:text-white"}`} />
                </button>
                {isLangOpen && (
                    ...
                    <div className="p-1.5">
                      {(["fa", "en"] as Locale[]).map((l) => (
                        <button
                          ...
                          className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-[13px] transition-all ${
                            locale === l
                              ? "bg-[#c4a882]/10 text-[#c4a882] font-semibold"
              ...
              <div ref={userRef} className="relative hidden lg:block">
                ...
                >
                  <User className={`w-[18px] h-[18px] transition-all duration-300 group-hover:scale-110 ${isScrolled ? "text-[#1a1a1a]/70 group-hover:text-[#c4a882]" : "text-white/80 group-hover:text-white"}`} />
                </button>
                {isUserOpen && (
                  <div className={`absolute top-full mt-2 w-64 bg-white rounded-2xl shadow-[0_12px_40px_rgba(0,0,0,0.12)] border border-[#1a1a1a]/5 overflow-hidden animate-fade-in ${isRTL ? "left-0" : "right-0"}`}>
                    <div className="p-2">
                      ...
                      {userMenuItems.map((item) => (
                        <a key={item.labelFa} href={item.href} onClick={() => setIsUserOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13px] text-[#1a1a1a] hover:bg-[#faf8f5] transition-all group">
                      ...
                      <div className="border-t border-[#1a1a1a]/8 mt-1 pt-1">
                        <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13px] text-red-500 hover:bg-red-50 transition-colors">
              ...
              {/* Mobile Toggle - Responsive sizing */}
              <button
                onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
                className={`lg:hidden w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center border transition-all duration-300 ${
                  isScrolled
                    ? "bg-transparent border-[#1a1a1a]/10 hover:bg-[#1a1a1a]/5"
                    : "bg-white/5 border-white/15 hover:bg-white/10"
                }`}
              >
                {isMobileMenuOpen ? (
                  <X className={`w-4 h-4 sm:w-5 sm:h-5 transition-colors ${isScrolled ? "text-[#1a1a1a]" : "text-white"}`} />
                ) : (
                  <Menu className={`w-4 h-4 sm:w-5 sm:h-5 transition-colors ${isScrolled ? "text-[#1a1a1a]" : "text-white"}`} />
                )}
              </button>
            </div>
          </div>
      ...
      <div dir={direction} className={`fixed inset-0 z-[60] lg:hidden transition-all duration-500 ${isMobileMenuOpen ? "opacity-100 visible" : "opacity-0 invisible pointer-events-none"}`}>
        ...
        <div className={`absolute top-0 h-full w-[85vw] max-w-[380px] bg-[#faf8f5] shadow-2xl transition-transform duration-500 ease-out ${
          ...
          <div className="flex flex-col h-full">
            {/* Mobile Header - Responsive padding */}
            ...
            <div className="p-4 border-b border-[#1a1a1a]/8 shrink-0">
              <div className="relative flex items-center h-11 sm:h-12 bg-[#1a1a1a]/[0.04] rounded-xl px-4 border border-[#1a1a1a]/8 focus-within:border-[#c4a882]/30 transition-colors">
            ...
            <div className="flex-1 overflow-y-auto py-2 overscroll-contain">
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <div key={link.key}>
                    <a
                      href={link.href}
                      onClick={(e) => { if (link.hasDropdown) e.preventDefault(); else setIsMobileMenuOpen(false); }}
                    ...
                    {link.hasDropdown && (
                      <div className="px-6 pb-2">
                        {applianceCategories.map((cat) => (
                          <a key={cat.id} href={`#${cat.id}`} onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-3 py-2.5 text-[#1a1a1a]/70 text-[13px] hover:text-[#c4a882] transition-colors">
              ...
              <div className="border-t border-[#1a1a1a]/8 mt-3 pt-3 mx-2">
                ...
                {userMenuItems.map((item) => (
                  <a key={item.labelFa} href={item.href} onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-3 px-4 py-3 text-[#1a1a1a] text-[14px] hover:bg-[#1a1a1a]/[0.03] rounded-xl transition-colors">
